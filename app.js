...
let cellNotes={};
function saveLembretes(){if(isConfigured){sbClient.from('rotina_config').upsert({key:'lembretes',data:cellNotes,updated_at:new Date().toISOString()}).then(({error})=>{if(error)setSyncStatus('error','Erro: '+error.message);else setSyncStatus('ok','Salvo');});}}
async function loadLembretes(){if(!isConfigured)return;const{data,error}=await sbClient.from('rotina_config').select('data').eq('key','lembretes').maybeSingle();if(!error&&data&&data.data)cellNotes=data.data;renderSemana();}
function addNote(key){if(!cellNotes[key])cellNotes[key]=[];cellNotes[key].push('');renderSemana();saveLembretes();}
function updNote(key,idx,v){cellNotes[key][idx]=v;saveLembretes();}
function delNote(key,idx){cellNotes[key].splice(idx,1);renderSemana();saveLembretes();}
...
async function insertItem(item){setSyncStatus('saving','Salvando...');const{error}=await sbClient.from('kanban_items').insert({title:item.title,type:item.type,resp:item.resp,prio:item.prio,day1:item.day1||null,day2:item.day2||null,period:item.period,due_date:item.date||null,obs:item.obs,status:item.status,steps:item.steps,status_log:item.statusLog});if(error){setSyncStatus('error','Erro: '+error.message);return;}setSyncStatus('ok','Salvo');}
async function updateItemRemote(id,fields){setSyncStatus('saving','Salvando...');const{error}=await sbClient.from('kanban_items').update(fields).eq('id',id);if(error){setSyncStatus('error','Erro: '+error.message);return;}setSyncStatus('ok','Salvo');}
async function deleteItemRemote(id){setSyncStatus('saving','Removendo...');const{error}=await sbClient.from('kanban_items').delete().eq('id',id);if(error){setSyncStatus('error','Erro: '+error.message);return;}setSyncStatus('ok','Removido');}
function subscribeRealtime(){if(!isConfigured)return;sbClient.channel('kanban_changes').on('postgres_changes',{event:'*',schema:'public',table:'kanban_items'},()=>loadData()).on('postgres_changes',{event:'*',schema:'public',table:'rotina_config'},()=>loadLembretes()).subscribe();}
...
let dragId=null;
function dgStart(e,id){dragId=id;e.dataTransfer.effectAllowed='move';if(e.target.classList)e.target.classList.add('dragging');}
function dgEnd(e){if(e.target.classList)e.target.classList.remove('dragging');}
function dgOver(e,el){e.preventDefault();el.classList.add('drag-over');}
function dgLeave(el){el.classList.remove('drag-over');}
function dropCell(e,day,per){e.preventDefault();const t=items.find(x=>x.id===dragId);if(t){t.day1=day;t.day2=day;t.period=per;if(isConfigured){updateItemRemote(t.id,{day1:day,day2:day,period:per}).then(loadData);}else{renderSemana();render();renderResumo();}}}
function dropPool(e){e.preventDefault();const t=items.find(x=>x.id===dragId);if(t){t.day1='';t.day2='';if(isConfigured){updateItemRemote(t.id,{day1:null,day2:null}).then(loadData);}else{renderSemana();render();renderResumo();}}}
function poolChip(x){return`<div class="chip ${TYPE_CLASS[x.type]}" draggable="true" ondragstart="dgStart(event,${x.id})" ondragend="dgEnd(event)" onclick="openModal(${x.id})"><div class="chip-title">${x.title}</div><div class="chip-meta"><span class="chip-resp"><i class="ti ti-user" aria-hidden="true" style="font-size:10px;margin-right:2px"></i>${x.resp}</span><span class="mini-prio ${PRIO_CLASS[x.prio]}">${x.prio}</span></div></div>`;}
function gridChip(x,dayKey,periodKey){const sl=spanLabel(x);const cont=isContinuation(x,dayKey,periodKey);return`<div class="chip ${TYPE_CLASS[x.type]} ${cont?'cont':''} ${x.status==='ok'?'done':''}" draggable="true" ondragstart="dgStart(event,${x.id})" ondragend="dgEnd(event)" onclick="openModal(${x.id})"><div class="chip-title">${x.title}</div><div class="chip-meta"><span class="chip-resp"><i class="ti ti-user" aria-hidden="true" style="font-size:10px;margin-right:2px"></i>${x.resp}</span><span class="mini-prio ${PRIO_CLASS[x.prio]}">${x.prio}</span>${sl?`<span class="span-tag"><i class="ti ti-arrows-horizontal" aria-hidden="true" style="font-size:9px"></i>${sl}</span>`:''}</div></div>`;}
function renderSemana(){
  const el=document.getElementById('rot-semana');if(!el)return;
  const semDia=items.filter(x=>!x.day1&&x.status!=='ok');
  const pool=`<div class="pool" id="pool" ondragover="dgOver(event,this)" ondragleave="dgLeave(this)" ondrop="dropPool(event)"><div class="pool-head"><i class="ti ti-inbox" aria-hidden="true"></i>Afazeres sem dia</div><div class="pool-list">${semDia.length?semDia.map(poolChip).join(''):'<div class="chip-empty">Tudo distribuído</div>'}</div><div class="pool-hint">Arraste um card para um dia. Arraste de volta aqui pra tirar.</div></div>`;
  const grid=`<div class="week-scroll"><div class="week-grid">`+DIAS.map(d=>`<div class="day-col ${d.k===todayKey?'today':''}"><div class="day-head">${d.l}${d.k===todayKey?'<span class="today-tag">hoje</span>':''}</div>${PERIODS.map(p=>{const key=d.k+'-'+p.k;const notes=cellNotes[key]||[];const chips=items.filter(x=>coversCell(x,d.k,p.k));return`<div class="period" ondragover="dgOver(event,this)" ondragleave="dgLeave(this)" ondrop="dropCell(event,'${d.k}','${p.k}')"><div class="period-label"><i class="ti ${p.icon}" aria-hidden="true" style="font-size:12px"></i>${p.l}</div><div class="notes-list">${notes.map((n,idx)=>`<div class="note-row"><input class="note-input" placeholder="lembrete..." value="${esc(n||'')}" onchange="updNote('${key}',${idx},this.value)"><button class="note-del" onclick="delNote('${key}',${idx})" aria-label="Remover"><i class="ti ti-x" aria-hidden="true"></i></button></div>`).join('')}</div><button class="add-note" onclick="addNote('${key}')"><i class="ti ti-plus" aria-hidden="true"></i>lembrete</button><div class="chips">${chips.length?chips.map(x=>gridChip(x,d.k,p.k)).join(''):'<div class="chip-empty">—</div>'}</div></div>`;}).join('')}</div>`).join('')+`</div></div>`;
  el.innerHTML=`<div class="dyn-layout">${pool}${grid}</div>`;
}
...
renderSemanal();
if(isConfigured){loadData();loadLembretes();subscribeRealtime();}else{renderSemana();renderResumo();render();setSyncStatus('error','Configure o Supabase no app.js');}
